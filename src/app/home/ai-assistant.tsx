...
export function AIAssistant({ onSuggest }: { onSuggest: any }) {
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Support Connection Error",
        description: err.message || "Connection lost. Try again.",
      });
      setMessages(prev => [...prev, { role: 'bot', text: "Maaf kijiye, main connect nahi ho pa raha hoon. Kripya apna internet aur API key check karein." }]);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <CardHeader className="bg-primary text-primary-foreground p-4 flex flex-row items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] opacity-70">Desi AI Assistant</p>
              </div>
            </div>
            ...
            <div className="h-80 overflow-y-auto p-4 space-y-4 bg-muted/30">
              {messages.length === 0 && (
                <div className="text-center py-10 opacity-50 space-y-2">
                  ...
                  <p className="text-xs">Namaste! Main LocalVyapar ka Support Bot hoon. <br/>Aap is app ko use kaise karein, mujhse pooch sakte hain!</p>
                </div>
              )}
              {messages.map((m, i) => (
